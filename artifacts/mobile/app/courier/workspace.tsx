/**
 * Courier Mission Workspace — Phase A7
 *
 * Map-first fullscreen operational screen. Replaces fragmented dashboard → offers →
 * missions → tracking flow with ONE unified workspace. Inspired by Uber Driver / Talabat.
 *
...
export default function CourierWorkspaceScreen() {
  ...
  // ETA float card top offset (below the top bar)
  ...
  return (
    <View style={s.root}>
      ...
      <View style={[s.topBar, { paddingTop: insets.top + 12 }]}>
        ...
        {/* Online/offline toggle — only when not busy on an active mission */}
        {!isBusy && (
          <Pressable
      ...
      {/* ─── ETA FLOAT (when on active mission) ─────────────────────────────── */}
      ...
      {/* ─── IDLE PANEL ─────────────────────────────────────────────────────── */}
      {workspaceState === "IDLE" && (
        <View style={[s.idlePanel, { backgroundColor: colors.card + "EE", borderColor: colors.border, paddingBottom: insets.bottom + 20 }]}>
